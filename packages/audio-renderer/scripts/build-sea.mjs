...
function printUsage() {
  process.stdout.write(
    [
      'Usage: node packages/audio-renderer/scripts/build-sea.mjs [options]',
      '',
      'Essential options:',
...
async function buildSeaBundle() {
  await build({
    ...
    resolve: {
      alias: {
        '@be-music/json': resolve(packageDir, '../json/src/index.ts'),
        '@be-music/parser': resolve(packageDir, '../parser/src/index.ts'),
        '@be-music/utils': resolve(packageDir, '../utils/src/index.ts'),
      },
    },
    build: {
      target: 'node22',
...
function resolvePostjectCliPath() {
  try {
    ...
    throw new Error(
      'postject is required for legacy SEA builds but was not found. Install it with `pnpm --filter @be-music/audio-renderer add -D postject`.',
    );
  }
}
...
async function main() {
  ...
  const outputPath =
    ...
    resolve(seaDir, process.platform === 'win32' ? 'be-music-audio-render.exe' : 'be-music-audio-render');
