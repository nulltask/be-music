...
function decodeUtf16BeText(buffer: Uint8Array): string {
...
function isAsciiText(buffer: Uint8Array): boolean {
...
function decodeText(buffer: Uint8Array, encoding: string, offset = 0): string {
  let text = new TextDecoder(encoding).decode(buffer.subarray(offset));
