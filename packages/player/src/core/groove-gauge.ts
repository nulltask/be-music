...
/**
 * LR2 gauge variants. `'GROOVE'` is the default (cumulative gain
 * from 20 %, clear at 80 %, soft floor at 2 %). The other modes
 * mirror `#SRC_BUTTON,type=40 / 41` cycling:
 *
 * - `'HARD'`   — start at 100 %, lose chunks on misses, fails at
 *   0 % (game continues but the chart is "FAILED" at end).
 * - `'DEATH'`  — start at 100 %, ANY miss / poor drops the
 *   gauge to 0 % immediately (no recovery).
 * - `'EASY'`   — gentler GROOVE: bigger gains, smaller losses,
 *   clear threshold lowered to 60 %.
 */
...
export function applyGrooveGaugeJudge(state: GrooveGaugeState, judge: GrooveGaugeJudgeKind): number {
  ...
  // DEATH gauge — any non-positive verdict zeroes the gauge so
  // subsequent renders / fail-detection see an instant flat-line.
...
function resolveGrooveGaugeInitial(type: GrooveGaugeType): number {
  // HARD / DEATH gauges start FULL — every chart is a survival
  // run from 100 % down. GROOVE / EASY follow the LR2 default of
  // 20 % so the gauge ramps up over the play.
...
function resolveGrooveGaugeMin(type: GrooveGaugeType): number {
  // GROOVE / EASY have a soft floor (2 %) so a mid-chart miss
  // streak doesn't lock the player out of recovery. HARD / DEATH
  // bottom out at 0 % so they can fail.
...
function resolveGrooveGaugeClearThreshold(type: GrooveGaugeType): number {
  ...
  // HARD / DEATH "clear" if you survive past 0 — any positive
  // gauge at end-of-chart counts. We reuse 0 + epsilon as the
...
function resolveHardGaugeDelta(_state: GrooveGaugeState, judge: GrooveGaugeJudgeKind): number {
  // HARD penalties are flat percentages — independent of TOTAL —
  // because the gauge starts at 100 % and the penalty model is
  // "how many BADs / POORs can you survive". Approximates LR2's
  // canonical values; re-tune if score-history calibration says
  // otherwise.
  if (judge === 'BAD') return -6;
  if (judge === 'POOR') return -10;
  if (judge === 'EMPTY_POOR') return -2;
  if (judge === 'GOOD') return 0.16;
  // PERFECT / GREAT — small recovery nudge, capped by the 100 %
  // ceiling.
  return 0.16;
}

function resolveDeathGaugeDelta(_state: GrooveGaugeState, judge: GrooveGaugeJudgeKind): number {
  // DEATH — any miss / poor zeroes the gauge (handled by the
  // post-clamp nudge in `applyGrooveGaugeJudge`). Hits give a
  // tiny token gain so the polyline reads as "alive" while the
  // chart still has playable notes.
...
function resolveEasyGaugeDelta(state: GrooveGaugeState, judge: GrooveGaugeJudgeKind): number {
  // EASY — gentler than GROOVE. ~1.2× baseline gains, ~50 %
  // smaller miss penalties.
