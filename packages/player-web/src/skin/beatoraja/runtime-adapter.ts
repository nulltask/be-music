...
import type { BeatorajaRenderContext } from './render.ts';
...
export class BeatorajaRuntimeAdapter {
  ...
  // `scene/lr2/select` for consistency. Diverges from upstream `PlayConfig.java:16`'s `1.0f` — see
   ...
   */
  private readonly earlyLateCounts: Record<
    'PERFECT' | 'GREAT' | 'GOOD' | 'BAD' | 'POOR' | 'MISS',
   ...
   */
  private deactivateLaneTimer(
    channel: string,
    resolver: (side: BeatorajaSide, lane: number) => number | undefined,
  ): void {
    const side = this.resolveSide(channel);
    const lane = this.resolveLane(channel);
    if (lane === undefined) return;
...
function createEmptyLaneJudgeRing(): Array<{ kind: number; at: number }> {
  return Array.from({ length: JUDGE_LANE_REF_RANGE }, () => ({ kind: -1, at: 0 }));
}
 ...
 * value space = 0).
 */
function resolveJudgeComboMetric(preferred: { width?: number; space?: number } | undefined): {
  width: number;
  space: number;
} {
  const width =
    typeof preferred?.width === 'number' && Number.isFinite(preferred.width) && preferred.width > 0
      ? preferred.width
      : 40;
  const space = typeof preferred?.space === 'number' && Number.isFinite(preferred.space) ? preferred.space : 0;
  return { width, space };
}
