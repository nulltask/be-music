...
import { BeatorajaRuntimeAdapter } from './runtime-adapter.ts';
...
describe('BeatorajaRuntimeAdapter — judgegraph type=1 (per-second judge state)', () => {
  function makeJudgeChart(
    events: ReadonlyArray<{ channel: string; pos?: [number, number]; measure?: number }>,
  ): import('@be-music/json').BeMusicJson {
    return {
      metadata: {
        bpm: 120,
        title: 'judge-state',
        rank: 2,
      } as unknown as import('@be-music/json').BeMusicJson['metadata'],
      events: events.map((e) => ({
        measure: e.measure ?? 0,
        channel: e.channel,
