 ...
 *
 * The associated `objectUrl` is created from the bytes' `Blob` and MUST be revoked (via `URL.revokeObjectURL`) once the
 * texture is disposed to free the underlying memory.
 */
...
/**
 * Tunables for the ffmpeg.wasm fallback path. Plumbed through from the host so end-users can trade encode time for
 * visual resolution at runtime (the demo wires this to a lil-gui pair).
 */
export interface VideoTranscodeOptions {
  /**
   * When set to a positive integer, the transcode pass downscales the source so neither side exceeds this many pixels
   * (aspect ratio preserved). Single-threaded libx264's per-frame cost scales linearly with width × height, so this is
   * the single biggest lever we have once `-preset ultrafast` is in effect. BMS BGA renders into a 256-px spec canvas
   * and even at 3× DPR tops out near 768 px on screen, so caps in the 480–720 range are visually transparent under
  ...
  /**
   * When true, the encode step uses the WebCodecs `VideoEncoder` API instead of libx264 in the wasm core. The decode
   * step still goes through ffmpeg.wasm because WebCodecs' `VideoDecoder` doesn't support the legacy codecs (MPEG-1,
   * VC-1, etc.) BMS BGA usually ships in.
   *
   * WebCodecs is hardware-accelerated where the browser's platform decoder/encoder is available — typically a 5–20×
   * encode-side speedup over the single-threaded wasm libx264. Falls back to the ffmpeg-only path silently when the
   * browser doesn't expose `VideoEncoder` (currently Safari < 17 / older Firefox builds), when `isConfigSupported`
   * rejects the requested codec parameters, or when any step of the WebCodecs pipeline throws.
   */
  useWebCodecs?: boolean;
}

/**
 * Loads a video file (`mp4` / `webm` / etc.) into a Pixi `Texture` backed by an HTML `<video>` element. The video
 * starts paused and muted — the caller is expected to seek + `.play()` it on cue.
 *
 * Falls back to {@link transcodeVideoToBrowserCodec} (via libav / ffmpeg.wasm) when the browser refuses to decode the
 * original bytes — typical for legacy BMS BGA shipping `.mpg` / MPEG-1 containers that no modern browser plays
 * natively. The transcode pass writes a temporary H.264 / yuv420p MP4 in memory and feeds that back through the same
 * `<video>` path, so the rest of the BGA pipeline (Pixi `VideoSource`, frame polling) is identical regardless of
 * whether a transcode happened.
 *
 * Returns `undefined` only when both the native decode AND the libav fallback fail — at that point we accept the asset
 * is unplayable and the BGA preloader simply skips it.
 */
...
/**
 * Browser-side check for the `VideoEncoder` half of WebCodecs. Used to gate the Web Codecs encode path before we even
 * hand bytes to ffmpeg for decoding — there's no point paying the decode cost if we can't encode the result with
 * WebCodecs.
 *
...
/**
 * Lazy-imports libav.js (ffmpeg.wasm) and remuxes / re-encodes `bytes` to MP4 / H.264 / yuv420p — the
 * lowest-common-denominator codec every modern browser plays natively. The libav module is a multi-megabyte WASM
 * bundle, so the import is intentionally deferred to here: a typical drop with only modern BGA video never pays the
 * download cost.
 *
 * `noworker: true` keeps everything on the main thread so the dev-server doesn't need to ship the COOP / COEP headers a
 * worker-based libav build would otherwise require. Transcoding is CPU-bound but happens during the BGA prepare phase
 * (which already overlaps the Decide splash), so the user never sees a frozen frame waiting for it.
 */
async function transcodeVideoToBrowserCodec(
  ...
  try {
    ...
    //   `-tune fastdecode,zerolatency` drops bidirectional prediction. `fastdecode` also swaps CABAC for CAVLC, which
    //   is ~10–15% faster to encode in addition to being faster to decode. - `-crf 30` trades a notch of quality for
    //   speed; BGA art tolerates it cleanly under nearest-filter scaling. - `-an` strips audio (BMS BGA's audio is
    //   irrelevant — `#WAV` drives chart audio). - `-movflags +faststart` puts the moov atom at the head of the file.
    //   Adds a second muxer pass but gives the `<video>` element predictable structure either way.
    //
    // `-vf scale=...` is only inserted when the host opted into resizing via `options.maxLongEdgePx`. See {@link
    // buildScaleFilterArg} for the exact filter graph.
    //
    // No `-threads` override: the wasm core is single-threaded (see `loadFfmpeg` for why we don't use core-mt), so
...
/**
 * Resolves the user's `maxLongEdgePx` knob into a strictly positive integer (or `undefined`). Anything else — `NaN`,
 * `0`, negative numbers — is treated as "no resize", which matches the GUI's `Off` row in the demo dropdown.
 */
...
/**
 * Builds the `-vf scale=...` filter string for a target long-edge cap. We use `min(iw,N)` / `min(ih,N)` so small inputs
 * (legacy 256-px BGA) pass through unchanged, then `force_original_aspect_ratio=decrease` collapses one edge to
 * preserve aspect when the input originally exceeded the cap. `force_divisible_by=2` keeps yuv420p chroma aligned, and
 * `flags=fast_bilinear` picks the cheapest scaler. Commas inside `min()` are escaped (`\\,`) so the filter-graph parser
 * doesn't read them as filter-chain separators.
 */
 ...
 *
 * - 1. ffmpeg.wasm decodes the source to raw `yuv420p` planes. We keep ffmpeg as the decoder because WebCodecs'
 *   `VideoDecoder` doesn't speak the legacy MPEG-1 / VC-1 / WMV codecs typical BMS BGA ships in. 2. JS wraps each
 *   decoded frame in a `VideoFrame` and feeds it to `VideoEncoder` (H.264 / avc1 baseline). On most platforms this hits
 *   the GPU / ASIC video encoder, taking encode time from ~1× realtime down to 5–20× realtime. 3. Encoded chunks land
 *   in `mp4-muxer`'s in-memory MP4 writer; once the encoder flushes, we hand the muxer's `ArrayBufferTarget` bytes back
 *   to the caller — same shape as the ffmpeg-encode path returns, so the downstream `<video>` blob plumbing is
 *   unchanged.
 *
 * Memory: decode and encode are interleaved in chunks rather than buffering the whole video as raw YUV. We can't get
 * true stream-style concurrency (the wasm worker's `exec()` call blocks the worker from servicing `readFile` until
 * decode completes, and there are no pipes in @ffmpeg/ffmpeg), but looping `ffmpeg.exec(['-ss', t, '-t', N, '-i',
...
async function transcodeViaWebCodecs(
  ...
  try {
    ...
    //
    // `hardwareAcceleration: 'prefer-hardware'` asks for the platform encoder (VideoToolbox on macOS, MediaFoundation
    // on Windows, MediaCodec on Android, …) but accepts the software fallback when none is available. We log the
    // negotiated mode for visibility.
...
/**
 * Pulls input geometry / frame rate / duration out of ffmpeg's stderr. The lines we want look like:
 *
 * `Duration: 00:02:02.00, start: 0.398189, bitrate: 5076 kb/s` `Stream #0:0(jpn): Video: vc1 ..., yuv420p, 1080x1080
 * [...], 30 tbr, 1k tbn`
 *
 * `tbr` (frame rate) and `WxH` come from the `Stream` line; `Duration:` carries the total length we use to size the
 * chunk loop. The rest of each line (codec, profile, SAR / DAR) is informative but doesn't affect the WebCodecs encoder
 * configuration.
 *
 * Returns `undefined` if any of geometry / frame rate / duration could not be parsed. The WebCodecs path bails out in
...
/**
 * Crude bitrate budget for the WebCodecs encoder, picked to land at roughly the same visual fidelity libx264's `-crf 30
 * -preset ultrafast` produces. The H.264 baseline profile we configure tops out around level 5.0 — enough headroom for
 * any BMS BGA dimensions we'd accept here.
 */
...
/**
 * Yields control to the event loop until the encoder's queue shrinks under `target`. Without this throttle, a fast wasm
 * decode loop can pile thousands of `VideoFrame`s into the encoder's queue and exhaust GPU video memory.
 */
...
/**
 * Lazy-loaded singleton — the `FFmpeg` class boots a Worker that downloads + instantiates the ~30 MB `@ffmpeg/core`
 * wasm. We keep the loaded instance around between calls so subsequent BGA videos transcode immediately instead of
 * paying the cold- start hit on every chart change.
 */
...
async function loadFfmpeg(): Promise<FfmpegInstance> {
  ...
  cachedFfmpegPromise = (async () => {
    ...
    // trips `ERROR_IMPORT_FAILURE`. The ESM bundle exports `createFFmpegCore` as `default` properly.
    //
    // We deliberately use `@ffmpeg/core` (single-threaded) over `@ffmpeg/core-mt`. The multi-threaded core 0.12.x has a
    // long-standing wasm bug where libx264's indirect calls crash every pthread worker with `RuntimeError: function
    // signature mismatch`, after which `ffmpeg.exec()` aborts with a non-Error throw that detonates inside the core's
    // own catch with `Cannot read properties of undefined (reading 'startsWith')`. Single-threaded encoding takes ~tens
    // of seconds for a typical BMS BGA but actually completes.
    //
    // The demo's Vite config installs a custom plugin (`be-music:ffmpeg-core`) that serves the ESM core
    // (`ffmpeg-core.js` / `.wasm`) at this stable URL prefix in dev and emits them as build assets in production.
    ...
    // Reset the promise cache on failure so a transient network blip doesn't permanently disable the fallback.
...
interface FfmpegInstance {
  ...
  /**
   * `@ffmpeg/ffmpeg` exposes an EventEmitter-style `on` / `off` for the underlying worker's `log` events. The WebCodecs
   * encode path uses this to capture stderr lines mid-decode (input geometry, frame rate) without displacing the
   * diagnostic listener `loadFfmpeg` already attaches.
   */
...
function pickInputExtension(path: string): string {
  ...
  // Preserve the original extension so libav picks the right demuxer (`.mpg` → mpegps, `.avi` → avi, `.wmv` → asf, …).
...
export interface LoadTextureOptions {
  ...
  /**
   * Treat pure-black pixels as transparent. Mirrors the BMS BGA "layer" convention (`packages/player/src/bga.ts`'s
   * `isOpaquePixel` for `mode === 'layer'`): the foreground BGA layer is composited over the base with `(0, 0, 0)`
   * acting as a chroma-key. Only enabled for layer-track decodes — base / POOR retain their black pixels because
   * they're the bottommost BGA layer (nothing visible behind them).
   */
  keyOutBlack?: boolean;
}

/**
 * Loads any LR2 / BGA asset (TGA, PNG, BMP, JPG, …) into a PixiJS `Texture`. Branches by file extension because TGA
 * isn't decoded by `createImageBitmap` in any browser.
 *
 * The legacy positional `transparentColor` argument is supported for backward compatibility — pass an
 * `LoadTextureOptions` object for the full feature set (`keyOutBlack`, future flags).
 */
