 ...
 * across the skin's elements gated on the lane's key-on timer (computed in {@link prepareSkin} via
 * `computeKeyOnFadeDurationsMs`); this constant kicks in only when a key-on slot has no skin element (skinless mode, or
 * a slot the skin doesn't author). 120 ms matches the LR2 default skin's typical key-on keyframe span and is a
 * perceptually comfortable decay for auto-judge feedback.
 */
const KEY_ON_FADE_OUT_MS = 120;

/**
 * "Fully on" hold time before {@link flashKeyOnTimer} hands the lane laser to the release-fade path. Without this hold
 * the sprite would only stay at peak alpha for the first frame (≈16 ms at 60 fps) before the fade tween kicked in,
 * which made auto-judged short-note flashes look like a single-frame blink. ~60 ms is enough to register visually as a
 * deliberate "tap" without lingering through the next note.
 */
 ...
 * `computeBombDurationsMs`. This constant is the fallback for skinless mode and for bomb slots the loaded skin doesn't
 * author. 150 ms matches the LR2 default 7-keys skin's bomb cycle.
 */
 ...
 * PixiGameplayView.getResultData}). Routed through the host into the result scene so it can render the LR2 result skin
 * without holding onto the gameplay view.
 *
 ...
 *
 * Used by the result scene's `Lr2GaugeChartElement` renderer — see `pixi-result.ts`. The series always contains at
 ...
 * count (0..`total*2`). The result scene normalises by the chart's theoretical max when drawing.
 */
...
export interface PixiGameplayViewOptions {
   ...
   * buffer). The snapshot is the same payload {@link PixiGameplayView.getResultData} returns — passed eagerly so the
   ...
   * that don't want a result screen can leave this unset and rely on `onExit` for both paths (legacy behaviour).
   */
   ...
   * behaviour most rhythm-game hosts ship.
   */
   ...
   *   flag so behaviour can be diff-tested directly.
   *
  ...
  /**
   * Optional source skin for the green note sprite drawn on top of each invisible note when {@link showInvisibleNotes}
   * is on. The renderer pulls `notes.note[3]` from this skin — index 3 is the green wide note in the LR2 default
   * `play_9.lr2skin` POP layout (Pop'n's third lane), so the convention is: pass the loaded 9-keys play variant here
   * and the invisible-note overlay uses the same green sprite a PMS chart would.
   *
   ...
   *
   * - `'HIDE'` (default) — judged notes vanish at the judgement instant. Matches the LR2 / beatoraja default behaviour
...
export class PixiGameplayView {
  ...
  /**
   * Sits above `noteLayer` / `bombLayer` and below `textLayer`. Holds the skin elements that should visually punch
   * through the note stream: the judgement plate, NOWCOMBO digits, and the AUTOPLAY indicator.
   */
   ...
   * include them all (depending on parser default), neither of which matches LR2 behaviour.
   */
   ...
   * inside the BAD window of both a mine and a regular note prefers the mine — matches LR2 behaviour where the mine
   * explodes first.
   */
   ...
   * lane it shares.
   *
   * Populated only when {@link PixiGameplayViewOptions.showInvisibleNotes} is on — this is a debug visualisation, not
  ...
  /**
   * Initialised in the constructor body to the baseline so the disc spins from t=0 even before any input arrives. Not
   * initialised inline because `private static readonly` fields can't be safely referenced at instance-field-init time
   ...
   * right keymode op (160..164), and to switch lane-channel detection to the PMS layout (`17` is a lane note, `22..25`
   * are 1P-side lanes) instead of the default IIDX layout. Defaults to `'7'` before any chart is loaded so the existing
   * 7K-flavoured fallbacks keep behaving as before for skinless / pre-prepare code paths.
   */
  private chartPlayVariant: '5' | '7' | '9' | '10' | '14' = '7';
  private laneX = new Map<string, { x: number; w: number; top: number; bottom: number }>();
  private textures = new Map<string, Texture>();
  /**
   * `performance.now()` value captured at `mount()`. Skin animations (LR2 timer 0/40/41 — scene-start / READY /
   * play-start) anchor here so the intro slide-ins, scratch turntable rotation, and similar visuals play from the
   ...
   *
   * `progress` is `seconds / totalSongSeconds` clamped to `[0, 1]`. Drives `Lr2GaugeChartElement` rendering on the
   * result scene — see `pixi-result.ts` for the polyline reveal animation.
   */
   ...
   * hit — they only stay synchronised on charts where every press triggers identical-timing hits on both sides (a
  ...
  /**
   * Per-key-on-timer (`100..117`) play-clock timestamp of an in-flight release fade. When set, `renderSkinImage` tapers
   * the sprite's `alpha` from 1 → 0 over the matching entry of {@link keyOnFadeDurationMs} starting at the recorded
   * value, so an LN release decays instead of popping off. {@link releaseKeyOnTimer} populates this; {@link
   ...
   * authored keyframes; {@link cleanupBombTimers} falls back to {@link BOMB_CLEANUP_FALLBACK_MS} for slots the skin
   * doesn't author (or for skinless mode).
   */
  ...
  /**
   * Per-gauge-rise / gauge-max timer (`42..45`) keyframe spans derived from the skin in {@link prepareSkin}. {@link
   * applyGaugeDelta} consults the rise entries (42 / 43) to time the gauge-increase flash; max entries (44 / 45) stay
   * active for as long as the gauge sits at 100 %, so their span is informational only.
   */
   ...
   * achievement so the LR2 FC timers (48 / 49) only fire once per play — replaying the chart resets this in
   * `prepareSong`. Note: AUTO mode reaches FC the moment the last note's auto-PERFECT lands, so the FC presentation
   * also plays during autoplay sessions (the player specifically asked for that behaviour).
   */
   ...
   * key-on, animation completion for bombs).
   */
  ...
  /**
   * Frame-rate sampling state. We accumulate frames over a one-second window and publish the rate to the LR2 RATE
   * NUMBER panel.
   */
  ...
  /**
   * Reveals the prepared scene, seeds the LR2 scene-stage timers, and starts the rAF loop. Must be called after {@link
   * prepare} resolves; calling it before will skip the chart-start scheduling because `audioContextStartTime` only
   * lands in the right ballpark when the audio context is up.
   *
  ...
  public start(): void {
    ...
    // on screen. `start()` only runs when `paused` is false and `pauseTotal` is still 0, so `playClock()` and
    ...
    // never animate).
    //
    // Timer 0 (scene start) fires immediately; timer 1 (`#STARTINPUT`) keeps its configured offset. Timer 40
    // (`#LOADEND`) and timer 41 (`#PLAYSTART`) — the events that drive "READY" / chart-start cues — are seeded later
    // via the `Promise.all` gates below so they wait for the BGA preload too.
    //
    // Timer 2 (FADEOUT) and timer 3 (CLOSE) are deliberately NOT seeded here even when the skin authored `#FADEOUT` /
    // `#CLOSE` durations: those directives describe scene-EXIT phase animations (= "when the scene starts to close,
    // fade out for N ms then close"), not offsets from scene mount. `beginExitSequence` is what stamps them at the
    // actual transition moment (ESC / chart end) so the LR2 default 7-keys "STAGE FAILED" plate (anchored to timer 3)
   ...
   * available to elements that gated on it (matches the pre-skin-timing default behaviour).
   */
  ...
  /**
   * Toggles the auto-pause-on-blur behaviour at runtime. Future `visibilitychange` / `blur` events will only auto-pause
  ...
  /**
   * Live setter for {@link PixiGameplayViewOptions.judgedNoteDisplay}. Mutates `this.options` so the per-frame
  ...
  /**
   * Live setter for {@link PixiGameplayViewOptions.showInvisibleNotes}. The invisible-note array is always extracted at
   ...
   * foreground (matching the LR2 desktop client behaviour). The user can still toggle manually with Space without
   ...
   * that lets the user opt into the LR2-desktop-style "pause when I Cmd-Tab away" behaviour.
   */
  ...
  /**
   * Window-level blur/focus fallback for the auto-pause behaviour. Some platforms (notably macOS with Chrome) keep the
   ...
   * in its currently-active state for {@link KEY_ON_FADE_OUT_MS} ms while a render-time alpha taper drives the visible
  ...
  /**
   * Pauses every BGA video element in the chart's video pool. The `<video>` element doesn't honour the audio-context
   ...
   * Behaviour branches on `note.longNoteMode`:
   *
   ...
   *
   * Snap is centred on zero (not baseline) deliberately. The baseline is the *idle* spin; while the user is actively
   ...
   *   ±delta speed — the visual analogue of a hand on the platter rocking it forward / back.
   ...
   * re-purpose as a frame-rate read-out per the user's request).
   */
   ...
   * at call time so skin elements gated on "FADEOUT" (typically a full-screen alpha overlay) play their authored
   * fade-out keyframes; when `#FADEOUT` ms have passed it seeds timer 3 so "CLOSE"-gated chrome (the LR2 default 7-keys
   * STAGE FAILED / CLEARED plate) plays before the actual transition fires.
   *
   * Idempotent — re-entry while a fade is in flight is a no-op, so a frantic second ESC press doesn't double-fire the
   * host callback. Skins with no `#FADEOUT` / `#CLOSE` directives collapse to immediate dispatch (no behavioural change
   * for skinless / non-LR2 demos).
   */
  /**
  ...
  private publishJudge(judge: JudgeKind, seconds: number, channel?: string): void {
    ...
    // timer regardless of the sourcing channel — the LR2 9-key skin authors only the 1P judge plate (channels 22..25
    // belong to the central / right half of the Pop'n nine-lane bank, not the second player).
  ...
  /**
   * Fires the LR2 full-combo timers (48 = 1P, 49 = 2P) the moment the player's running combo equals the chart's
   * playable note count — i.e. every note has been hit GREAT-or-better and the chain never broke. Latches
   * `fullComboFired` so subsequent judges don't re-trigger the timer (which would replay the skin's FC slide-in
   ...
   *
   * Both 48 and 49 are stamped at the same `performance.now()` because we don't yet model per-side combos. For DP this
   * is correct (one player, one combo). Battle mode would split.
   */
   ...
   *
   * Honours the bmson 1.0.0 `note.c` (continuation flag): when `c === true`, a still-playing instance of the same
   ...
   * press triggers a fresh playback" behaviour.
   */
  private playSample(note: RuntimeNote): void {
    ...
    // historical play-from-zero behaviour kicks in.
  ...
  /**
   * One-shot bomb timer cleanup. Runs every frame so the LR2 bomb timers (50–69) stop being "active" once the skin's
   * authored explosion keyframes have played out. The per-timer duration is pre-computed in {@link prepareSkin} via
   * `computeBombDurationsMs` (longest `time` keyframe across elements gated on each timer), with
   * `BOMB_CLEANUP_FALLBACK_MS` covering skinless / unauthored slots. Without this retirement, the skin's `#DST_IMAGE`
   * (typically `loop=-1` "play once and clamp") would keep displaying the last frame of the explosion forever.
   */
   ...
   * `laneX` so the mask exactly covers the lane area regardless of how the skin laid the lanes out (single-side, DP,
   * etc.).
   *
  ...
  private renderBga(seconds: number): void {
    ...
    for (const bga of visibleBgas) {
      ...
      const drawLayer = (
        ...
        // exactly the same key normalisation the parser applies to the `chart.bms.argb` / `chart.bms.exBmp` maps, so
   ...
   * the LR2 default 7-keys skin). The skin's source frame already encodes the colour; we only need to honour the
  ...
  /**
   * Renders the judgement plate + NOWCOMBO digits as a single horizontally centred assembly. The two are drawn together
   * so the relative gap stays stable while the whole group slides left/right to centre on the lane area as the combo
   * gets longer.
   *
  ...
  /**
   * Returns the horizontal centre of one side's play-field area (in design pixels) derived from the LR2 skin's
  ...
  private renderNotes(seconds: number, _height: number): void {
    ...
    if (this.options.showInvisibleNotes && this.invisibleNotes.length > 0) {
      ...
      // rectangle so the overlay still reads.
  ...
  /**
   * Renders one landmine sprite. Tries the LR2 skin's `#SRC_NOTE` `mine` slot first (the skin's authored mine graphic,
  ...
  /**
   * Draws horizontal measure lines on the playfield at every `#MEASURE` boundary. When the LR2 skin defines `#SRC_LINE`
   * / `#DST_LINE`, we use its texture & geometry; otherwise we fall back to a thin white bar spanning the lane area.
   */
   ...
   * counted here so notes still render in their normal colour during autoplay demonstrations, matching the LR2
   * reference.
   */
   ...
   * The body sprite is taken from the LR2 skin per lane index when available; otherwise we fall back to a tinted
   * rectangle.
   */
...
/**
 * Note colour for the no-skin fallback path, mirroring the IIDX / LR2 default convention:
 *
 ...
 * non-finite / negative input collapses to 0 so the historical "play from t=0" behaviour stays the default fallback.
 */
 ...
 * (seek + cap), and we want to match the historical behaviour for the two- and three-arg cases when slicing isn't in
 * play.
 *
