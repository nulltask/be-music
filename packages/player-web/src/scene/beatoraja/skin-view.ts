...
/**
 * Diagnostic state for a gauge entry — emits one structured log per "interesting transition"
 * (first state seen, mode flip, value/border drop to zero, all-cells-hidden frame). Lets
...
interface GaugeDiagState {
  /** Logged at least once after the entry resolves — tells us the build outcome. */
  initialDeclLogged: boolean;
  /** Last (mode, value-bucket, border-bucket) snapshot — log when this changes. */
  lastSnapshotKey: string;
  /** Frames in a row where every cell hid (= visual black). Logged on the first run. */
  allHiddenLogged: boolean;
}
...
interface GaugeEntry {
  ...
  /** Per-frame diagnostic-log gating. See {@link GaugeDiagState} for the rationale. */
  diag: GaugeDiagState;
}
...
export class BeatorajaPlaySkinView {
   ...
   * `scene/lr2/gameplay.ts:469`: an off-stage skin destination (slide-ins authored from
  ...
  private readonly resolveTimingJudgeWindowsMs: () =>
    ...
    | undefined;
  private readonly resolveJudgeStateBuckets: (
    type: number,
  ) => { buckets: ReadonlyArray<ReadonlyArray<number>>; maxCount: number; totalMs: number } | undefined;
  ...
  constructor(options: BeatorajaPlaySkinViewOptions) {
    ...
    this.customEvents = normalizeBeatorajaCustomEvents((options.skin as { customEvents?: unknown }).customEvents);
    this.customTimers = normalizeBeatorajaCustomTimers((options.skin as { customTimers?: unknown }).customTimers);
    ...
    const collectTimingVisualizers = (input: unknown): void => {
      for (const tv of normalizeBeatorajaTimingVisualizers(input)) {
        if (!claimedBy(tv.id) && !bpmGraphById.has(tv.id) && !judgeGraphById.has(tv.id) && !gaugeGraphById.has(tv.id)) {
    ...
    const expandedJudgeDestinations = expandBeatorajaJudgeDestinations(judges, valueById, referencedJudgeIds);
  ...
  private buildGaugeEntry(
    ...
    for (const nodeId of element.nodes) {
      if (!nodeTextures.has(nodeId)) missingNodes.push(String(nodeId));
    }
    // eslint-disable-next-line no-console
    console.log(
      '[beatoraja-view] gauge entry built',
      JSON.stringify({
        groupId: String(group.id),
        parts: element.parts,
        animationType: element.type,
        animationRange: element.range,
        cycle: element.cycle,
        nodeIds: element.nodes.slice(0, 24).map(String),
        nodeCount: element.nodes.length,
        resolvedNodes: resolvedNodes.slice(0, 24),
        missingNodeCount: missingNodes.length,
        missingNodes: missingNodes.slice(0, 12),
      }),
    );
    return {
      kind: 'gauge',
      ...
      overlay,
      nodeTextures,
      diag: { initialDeclLogged: false, lastSnapshotKey: '', allHiddenLogged: false },
    };
  }
    ...
    if (signature !== entry.lastSignature) {
      ...
      if (entry.element.backTexOff === 0) {
        ...
        for (let i = 10; i < maxCount; i += 10) {
          const bandColor = ((Math.floor(0.007 * i * 255) << 16) | (Math.floor(0.007 * i * 255) << 8) | 0) >>> 0;
    ...
    if (signature !== entry.lastSignature) {
      ...
      if (props.height > 0 && props.width > 0) {
        ...
        for (const band of bands) {
          ...
          if (newLeft < leftEdge) {
            graphics.rect(newLeft, 0, leftEdge - newLeft, props.height).fill({ color: band.color, alpha: 1 });
            leftEdge = newLeft;
          }
          if (newRight > rightEdge) {
            graphics.rect(rightEdge, 0, newRight - rightEdge, props.height).fill({ color: band.color, alpha: 1 });
            rightEdge = newRight;
          }
        }
  ...
  private updateGaugeEntry(
    ...
    // Per-frame diagnostic counters. Drives the "all cells hid → visual black" log so we can
    // correlate a black gauge frame with the picker's pick decisions (= which nodeId was asked
    // for, vs which the texture map actually has). Only emitted on the FIRST occurrence per
    // entry (gated by `entry.diag.allHiddenLogged`) to avoid console flooding.
    let visibleCellCount = 0;
    let pickerEmptyCount = 0;
    let textureMissingCount = 0;
    let lastMissingNodeId: BeatorajaImageId | undefined;
    for (let i = 0; i < entry.cells.length; i += 1) {
      ...
      if (texture === undefined) {
        cell.visible = false;
        textureMissingCount += 1;
        lastMissingNodeId = pick.nodeId;
        continue;
      }
      cell.visible = true;
      visibleCellCount += 1;
 ...
 * player progresses.
 */
const JUDGE_STATE_COLORS: ReadonlyArray<number> = [0x555555, 0x0088ff, 0x00ff88, 0xffff00, 0xff8800, 0xff0000];
