...
import {
  runEngineDriver,
  type EngineDriverAudioContext,
  type EngineDriverResult,
} from '../../runtime/engine-driver.ts';
import { BeatorajaRuntimeAdapter } from '../../skin/beatoraja/runtime-adapter.ts';
...
export interface PixiBeatorajaGameplayViewOptions {
   ...
   *
   * Mirrors the LR2 gameplay scene's same wiring (`scene/lr2/gameplay.ts:1700`'s
...
export class PixiBeatorajaGameplayView implements PixiScene {
  ...
  // in `scene/lr2/select` for consistency, NOT upstream `PlayConfig.java:16`'s `1.0f`. See the
  ...
  enter(host: PixiSceneHost): void {
    ...
    this.enginePromise = runEngineDriver({
      ...
      // `scene/lr2/gameplay.ts:1700`. Without this a `#PLAYER 1` PMS chart that doesn't use channel 17
      ...
      engineOptions: {
        ...
        // Play-log recording — host-supplied `engineOptions.recordPlaylog` fields (gauge pick, chart hash, ...)
        // are preserved; the view adds what it owns: the DP-flip intent it applied to the chart, and the chart
        // path in `native` so a dropped play-log can be matched back to its song.
        recordPlaylog: {
          ...this.options.engineOptions?.recordPlaylog,
          ...(this.options.dpFlip !== undefined ? { dpFlip: this.options.dpFlip } : {}),
          ...(this.options.chartPath !== undefined
            ? {
                native: {
                  ...this.options.engineOptions?.recordPlaylog?.native,
                  chartPath: this.options.chartPath,
                },
              }
            : {}),
        },
  ...
  private readonly handleKeyDown = (event: KeyboardEvent): void => {
    ...
    if (event.key === 'j' || event.key === 'J' || event.key === 'k' || event.key === 'K') {
      ...
      this.flashCoverIndicator(enable ? `Hidden cover ${Math.round(next * 100)}%` : 'Hidden cover OFF');
      return;
    }
 ...
 * shape as the resolver in `scene/beatoraja/decide.ts`; lifted here so play scenes that
 ...
 * a non-zero space.
 */
function readJudgeComboMetrics(skin: BeatorajaSkin): {
  1: { width: number; space: number };
  2: { width: number; space: number };
} {
  const result = {
    1: { width: 40, space: 0 },
    2: { width: 40, space: 0 },
 ...
 * the LR2 path's `extractChartExtension` (`scene/lr2/gameplay.ts:5602`) so both gameplay surfaces
