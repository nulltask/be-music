...
import {
  drawDefaultBomb,
  drawDefaultLongNoteBody,
  drawDefaultMine,
  drawDefaultNoteBody,
  paintDefaultLane,
  paintDefaultLaneGridRight,
...
export class PixiGameplayView {
  ...
  private renderDefaultBombs(): void {
    ...
    for (const [channel, startedAt] of this.bombStartedAt) {
      ...
      graphic.alpha = 1;
      drawDefaultBomb(graphic, lane, progress);
    }
  }
  ...
  private renderLanes(width: number, height: number): void {
    ...
    this.laneChannels.forEach((channel, index) => {
      ...
      const tone = resolveDefaultLaneTone(channel, fallbackLaneIndex, this.chartPlayVariant);
      const beat = this.currentBeat(this.currentSeconds());
    ...
    if (!skin && this.laneX.size > 0) {
      ...
      paintDefaultLaneGridRight(this.laneLayer, { top: gridTop, bottom: gridBottom, right: gridRight });
    }
  }
  ...
  private renderMineNote(
    ...
    drawDefaultMine(graphic, lane.x, y, lane.w);
  }
  ...
  private renderMeasureLines(currentBeat: number, pixelsPerBeat: number): void {
    ...
    for (let beatIndex = firstBeatIndex; beatIndex < beats.length; beatIndex += 1) {
      ...
      graphic.rect(x0, Math.round(y), x1 - x0, 1).fill({ color: DEFAULT_THEME.measure, alpha: 0.32 });
    }
  }
  ...
  private renderSingleNote(
    ...
    drawDefaultNoteBody(
      ...
      y,
      Math.max(4, lane.w - 2),
      resolveDefaultLaneTone(channel, laneIndex, this.chartPlayVariant),
    );
  }
