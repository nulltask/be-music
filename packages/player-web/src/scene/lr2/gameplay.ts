...
import { loadSkinAssetTexture, loadTextureFromBytes, loadVideoTextureFromBytes } from '../../skin/lr2/textures.ts';
import {
...
import {
  defaultNoteTone,
  drawDefaultBomb,
  drawDefaultLaneColumn,
  drawDefaultLaneGridRight,
  drawDefaultLnBody,
  drawDefaultMeasureLine,
  drawDefaultMine,
  drawDefaultNoteBody,
} from '../default/playfield.ts';
...
export class PixiGameplayView {
   ...
   * lane it shares.
   *
   * Populated only when {@link PixiGameplayViewOptions.showInvisibleNotes} is on — this is a debug visualization, not
   ...
   * result scene — see `scene/lr2/result.ts` for the polyline reveal animation.
   */
   ...
   *
   * Seeded to 2.0 to match `DEFAULT_PLAY_OPTIONS.hiSpeed` in `scene/lr2/select` — the select view always passes
   ...
   * beatoraja's `PlayConfig.java:16` (`hispeed = 1.0f`); see the rationale in `scene/lr2/select.ts`.
   */
  private hiSpeed = 2.0;
  /**
  ...
  private prepareSong(song: BrowserSongEntry): void {
    ...
    // path's `composeBeatorajaEngineOptions` wiring at `scene/beatoraja/gameplay.ts:1338-1340`. Without this:
   ...
   * host callback. Skins with no `#FADEOUT` / `#CLOSE` directives collapse to immediate dispatch (no behavioral change
  ...
  private renderDefaultBombs(): void {
    ...
    for (const [channel, startedAt] of this.bombStartedAt) {
      ...
      graphic.alpha = 1;
      drawDefaultBomb(graphic, lane, elapsed, BOMB_CLEANUP_FALLBACK_MS);
    }
  }
  ...
  private resolveSkinlessGameplayChromeRuntime(): SkinlessGameplayChromeRuntime {
    ...
    return {
      ...
      sceneElapsedMs: this.sceneStartTime === 0 ? 0 : Math.max(0, this.playClock() - this.sceneStartTime),
      progressRatio: (() => {
        const totalSeconds = this.resolveSongDurationSeconds();
  ...
  private renderLanes(width: number, height: number): void {
    ...
    this.laneChannels.forEach((channel, index) => {
      ...
      if (skin) {
        ...
        // itself (driven by `#DST_IMAGE` + key-on / judgement timers). Drawing our own colored rectangles on top of
      ...
      const tone = defaultNoteTone(channel, fallbackLaneIndex, this.chartPlayVariant);
      const beat = this.currentBeat(this.currentSeconds());
      drawDefaultLaneColumn(
        this.laneLayer,
        { x, w, top, bottom },
        ...
        {
          scratch: scratchLane,
          laserAlpha: this.resolveFallbackLaneLaserAlpha(channel),
    ...
    if (!skin && this.laneX.size > 0) {
      ...
      drawDefaultLaneGridRight(this.laneLayer, gridRight, gridTop, gridBottom);
    }
  }
  ...
  private renderMineNote(
    ...
    drawDefaultMine(graphic, lane.x + 2, y, Math.max(4, lane.w - 4));
  }
  ...
  private renderMeasureLines(currentBeat: number, pixelsPerBeat: number): void {
    ...
    for (let beatIndex = firstBeatIndex; beatIndex < beats.length; beatIndex += 1) {
      ...
      drawDefaultMeasureLine(graphic, x0, x1, y);
    }
  }
  ...
  private renderSingleNote(
    ...
    if (skinNote && baseTexture) {
      ...
      // non-animated notes (cycle=0) this returns cell (0,0) which matches the static behavior.
    ...
    drawDefaultNoteBody(
      ...
      y,
      Math.max(4, lane.w - 2),
      defaultNoteTone(channel, laneIndex, this.chartPlayVariant),
    );
  }
