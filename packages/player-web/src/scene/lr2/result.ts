...
import { renderDefaultResultChrome } from '../default/result-render.ts';
 ...
 *
 * Notable difference from `scene/lr2/select.ts`: the result screen has its **own** rank op range (300..307 for the 1P
...
export class PixiResultView {
  ...
  public async mount(host: PixiSceneHost, result: PixiGameplayResultData): Promise<void> {
    ...
    if (!this.options.skin) {
      // Ice-clock rank punch lands after the banner/metrics assemble, not after LR2's 3s chart-draw window.
      this.setTimer(
        () => {
          if (this.disposed || this.timerStartedAt.has(151)) return;
          this.timerStartedAt.set(151, performance.now());
        },
        560,
      );
  ...
  private renderFallbackPanel(designWidth: number, designHeight: number): void {
    ...
    renderDefaultResultChrome(this.fallbackLayer, {
      designWidth,
      designHeight,
      cleared: result.cleared,
      title: result.song.title,
      artist: result.song.artist || result.song.subtitle || '',
      rank: resolveRankLabel(result.score.exScore, result.score.total),
      rate: computeScoreRate(result.score) * 100,
      ...
      notes: result.score.total,
      perfect: result.score.perfect,
      great: result.score.great,
      good: result.score.good,
      bad: result.score.bad,
      poor: result.score.poor,
      gaugeHistory: result.gaugeHistory.map((sample) => ({ x: sample.progress, y: sample.value / 100 })),
      scoreHistory: result.scoreHistory.map((sample) => ({
        x: sample.progress,
        y: exMax > 0 ? sample.exScore / exMax : 0,
      })),
      rankRevealed: this.timerStartedAt.has(151),
      rankElapsedMs: this.elapsedSinceTimer(151),
