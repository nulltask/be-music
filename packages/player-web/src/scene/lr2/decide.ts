 ...
 * result screen (e.g. difficulty / key-mode rather than clear-status). Sharing `scene/lr2/result.ts`'s renderer would have
 * meant teaching it to handle both data shapes; the cleaner cut is a sibling module that follows the same Pixi-scene
 * pattern.
 */
/**
 * 640×480 fallback canvas to match LR2 default `decide.lr2skin`'s native dimensions. See `scene/lr2/select` for the
...
/**
 * Element op set flipped per-song on the decide screen. Mirrors `RESULT_DYNAMIC_OPS` in `scene/lr2/result.ts` but keeps only
