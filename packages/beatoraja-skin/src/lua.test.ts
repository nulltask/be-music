...
describe('evaluateBeatorajaLuaSkin', () => {
  ...
  it('runtimeContext is optional — entry script still loads when the host omits it', () => {
    ...
    const result = evaluateBeatorajaLuaSkin({
      entry: enc(['local main_state = require("main_state")', 'return { active = main_state.option(151) }'].join('\n')),
      modules: [],
      skinConfig: { offset: 0, option: {}, file: {} },
    });
    if (!result.ok) throw new Error(result.error.message);
  ...
  it('audio_play returns false (and does not crash) when the host omits the callback', () => {
    ...
    const result = evaluateBeatorajaLuaSkin({
      entry: enc(
        ['local main_state = require("main_state")', 'return { ok = main_state.audio_play("click.ogg") }'].join('\n'),
      ),
      modules: [],
      skinConfig: { offset: 0, option: {}, file: {} },
    });
  ...
  it('exposes timer_util.timer_observe_boolean as a TimerProperty closure tied to a Lua boolFn', () => {
    ...
    if (!result.ok) throw new Error(result.error.message);
    const value = result.value as { fn: import('./lua.ts').BeatorajaLuaFunctionValue; off: number };
  ...
  it('audio_play with empty path short-circuits without invoking the host callback', () => {
    ...
    let invoked = false;
    const result = evaluateBeatorajaLuaSkin({
      entry: enc(['local main_state = require("main_state")', 'return { ok = main_state.audio_play("") }'].join('\n')),
      modules: [],
      skinConfig: { offset: 0, option: {}, file: {} },
      runtimeContext: {
        audioPlay: () => {
  ...
  it('places a placeholder in the cache before running a module body so circular requires terminate (audit 3.11)', () => {
    ...
    const moduleA = enc(['local a = {}', 'a.name = "a"', 'a.b = require("modb")', 'return a'].join('\n'));
    const moduleB = enc(
      [
        'local b = {}',
        'b.name = "b"',
    ...
    const result = evaluateBeatorajaLuaSkin({
      entry: enc(['local moda = require("moda")', 'return { aname = moda.name, bname = moda.b.name }'].join('\n')),
      modules: [
