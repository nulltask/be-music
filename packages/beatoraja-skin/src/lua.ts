...
function dayOfYear(d: Date, useUtc: boolean): number {
  const start = useUtc ? Date.UTC(d.getUTCFullYear(), 0, 1) : new Date(d.getFullYear(), 0, 1).getTime();
 ...
 */
function pushBuiltinLuaModule(L: lua_State, name: string, owner: LuaRuntimeOwner, exposeMainState: boolean): boolean {
  switch (name) {
    case 'main_state':
...
function pushMainStateStub(L: lua_State, owner: LuaRuntimeOwner, exposeMainState: boolean): void {
  ...
  const setNumberAccessor = (
    key: string,
    resolve: (ctx: BeatorajaLuaRuntimeContext, state: lua_State) => number | undefined,
  ): void => {
...
function pushOffsetTable(L: lua_State, populated: BeatorajaLuaSkinConfig['offset']): void {
  ...
  if (populated && typeof populated === 'object' && !Array.isArray(populated)) {
    for (const [name, value] of Object.entries(
      populated as Readonly<Record<string, Partial<BeatorajaLuaOffsetValue>>>,
    )) {
