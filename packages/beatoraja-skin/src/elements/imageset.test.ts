...
import { normalizeBeatorajaImagesets } from './imageset.ts';

describe('normalizeBeatorajaImagesets', () => {
  it('parses minimal imageset (id + ref + images)', () => {
    const out = normalizeBeatorajaImagesets([{ id: 'modeset', ref: 11, images: ['allkeys', '5keys', '7keys'] }]);
    expect(out).toEqual([
      {
        id: 'modeset',
        ref: 11,
  ...
  it('parses act / click click-event fields (audit 2.5)', () => {
    ...
    const out = normalizeBeatorajaImagesets([{ id: 'modeset', ref: 11, images: ['a', 'b'], act: 11, click: 1 }]);
  ...
  it('coerces non-string / non-number image entries away (defensive)', () => {
    const out = normalizeBeatorajaImagesets([{ id: 'mix', ref: 0, images: ['valid', null, 42, true, 'also-valid'] }]);
