...
function makeGauge(
  overrides: Partial<BeatorajaGaugeElement> & Pick<BeatorajaGaugeElement, 'nodes'>,
...
describe('pickBeatorajaGaugeNode (non-FLICKERING)', () => {
  ...
  it('uses ASSISTEASY slab when state.mode = ASSISTEASY', () => {
    const pick = pickBeatorajaGaugeNode(gauge, 4, state(50, 100, 80, BEATORAJA_GAUGE_MODE.ASSISTEASY), 0);
...
describe('pickBeatorajaGaugeNode (FLICKERING)', () => {
  const nodes = ['n-fill-a', 'n-fill-b', 'n-tail-a', 'n-tail-b', 'n-top-a', 'n-top-b'];
  ...
  it('lit cells use offset 0 (filled), off cells use offset 2 (empty)', () => {
    ...
    const pick = pickBeatorajaGaugeNode(gauge, 4, state(100, 100, 80, BEATORAJA_GAUGE_MODE.ASSISTEASY), 0);
    ...
    const empty = pickBeatorajaGaugeNode(gauge, 7, state(20, 100, 80, BEATORAJA_GAUGE_MODE.ASSISTEASY), 0);
  ...
  it('emits a flickerOverlayId at the topmost lit cell (i == notes)', () => {
    ...
    const tail = pickBeatorajaGaugeNode(gauge, 3, state(50, 100, 80, BEATORAJA_GAUGE_MODE.ASSISTEASY), 0);
