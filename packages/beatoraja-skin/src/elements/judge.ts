...
import { flattenBeatorajaElements } from './base.ts';
...
import type { BeatorajaValueElement } from './value.ts';
...
export function expandBeatorajaJudgeDestinations(
  ...
  for (const judge of judges) {
    ...
    for (let i = 0; i < numberLimit; i += 1) {
      ...
      const originalValue = valueId !== undefined ? valuesById?.get(valueId as BeatorajaImageId) : undefined;
      const valueDigit = originalValue !== undefined ? Math.max(1, Math.trunc(originalValue.digit)) : undefined;
      const folded = parent !== undefined ? foldChildDestIntoParent(child, parent, valueDigit) : child;
    ...
    if (Array.isArray(parentObj.if) && parentObj.value !== undefined) {
      ...
      if (typeof inner.x === 'number' || typeof inner.y === 'number') {
        ...
        for (const ckf of childPositionKfs) {
          ...
          const preShiftX = valueDigitForPreShift !== undefined ? (numberOrZero(ckf.w) * valueDigitForPreShift) / 2 : 0;
