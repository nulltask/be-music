...
describe('normalizeBeatorajaDestinations', () => {
  ...
  it('passes through plain elements that have `if` but no `value` / `values`', () => {
    ...
    const out = normalizeBeatorajaDestinations([
      { id: 'plain', dst: [{ if: [999], time: 0, x: 10, y: 20, w: 30, h: 40 }] },
    ])[0]!;
...
describe('normalizeBeatorajaDestinations — mouseRect (audit C-12)', () => {
  ...
  it('treats missing / malformed mouseRect as undefined', () => {
    const noField = normalizeBeatorajaDestinations([{ id: 'a', dst: [{ time: 0, x: 0, y: 0, w: 1, h: 1 }] }]);
...
describe('destination.stretch (audit 2.12)', () => {
  it('defaults to 0 (= STRETCH) when no stretch is authored', () => {
    const groups = normalizeBeatorajaDestinations([{ id: 'bg', dst: [{ time: 0, x: 0, y: 0, w: 100, h: 100 }] }]);
