...
describe('readDxaArchive', () => {
  ...
  it('accepts a synthesized minimal V3 archive (single uncompressed file)', () => {
    // Build a minimal V3 archive in memory:
    //   - Header (24 bytes)
    //   - Data area: file data 'hello world' (11 bytes) at offset 24
    //   - FileNameTable @ 24 + 11 = 35 (8-byte aligned → 40 actually, but
    //     we lay it tightly here — the spec doesn't enforce alignment for
    //     reads, just the encoder chooses 8-byte alignment in practice).
    //   - FileTable @ FileNameTableHead + 0
    //   - DirectoryTable @ FileTableHead + 2 entries × 44 = 88
    //
    // Two file-table entries: the implicit root-directory self-entry
    // (empty name, dir attr) + one regular file entry pointing at
    // the 11-byte payload.
    ...
    // Round file-name table to a multiple of 4 bytes so the next entry
    // aligns; here we have 1 entry total + the leading "" entry of the root.
    ...
    // Root entry — name @ 0 (empty), attributes = 0x10 (dir),
    // dataSize = 0, dataHead = 0, pressDataSize = 0xFFFFFFFF (n/a)
    ...
    // Regular file entry — name @ 4 (start of "file.txt" sub-entry),
    // attr = 0x20 (archive), dataHead = 0, dataSize = 11,
    // pressDataSize = 0xFFFFFFFF (uncompressed)
