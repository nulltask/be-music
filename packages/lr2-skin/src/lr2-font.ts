/**
 * LR2 bitmap font support — `.lr2font` text-format parser, glyph
 * map builder, and LR2 → Unicode character-code conversion.
 *
 * `.lr2font` directives (per `docs/LR2SkinHelp.md` line 2099+):
 *
 * - `#S <n>` — base font size (px). Reference height for the
 *   glyph cells; DST height is divided by this to compute the
 *   render-time scale factor.
 * - `#M <n>` — character spacing (px), can be negative for tight
 *   text where successive glyphs overlap.
 * - `#T <gr> <relative-path>` — declares image #`gr` referencing
 *   a PNG / BMP. Path is RELATIVE TO THE `.lr2font` FILE (not the
 *   skin CSV like `#IMAGE`).
 * - `#R <charCode> <gr> <x> <y> <w> <h>` — glyph rect: take from
 *   image `gr` the rect `(x, y, w, h)` and treat it as the bitmap
 *   for LR2 character code `charCode`. Glyph height is the base
 *   size (`#S`); a height different from `#S` is rescaled at
 *   render time.
 *
 ...
 *
 * The two SJIS branches encode the upper / lower SJIS double-byte
 * ranges (`0x81xx..0x9Fxx` and `0xE0xx..0xFCxx`), with the offset
 * picked so the result fits in a small dense int range. We round-
 * trip through a runtime-built UTF-16 ↔ SJIS table for full
 * Japanese coverage without bundling a static SJIS table.
 */
...
/**
 * Parses a `.lr2font` source text into the structured form. Lines
 * are tokenised on whitespace; unknown directives are silently
 * skipped so future `#X` keywords don't break the parse. Unparseable
...
function stripLineComment(line: string): string {
  // LR2 conventionally uses `//` (C++) line comments inside skin
  // files. `.lr2font` follows the same convention for compatibility
  // with FontUtil-generated output.
...
/**
 * Converts a single Unicode code point (UTF-16 BMP) to an LR2 char
 * code, or `undefined` when the character has no SJIS encoding (so
 * the renderer can substitute a placeholder rect).
 *
 * Lazy-built SJIS encoding table — first call costs ~5-10 ms while
 * we walk every double-byte SJIS sequence and decode it via the
 * platform's `TextDecoder('shift-jis')`. Subsequent calls are O(1)
 * map lookups.
 */
export function unicodeToLr2CharCode(char: string): number | undefined {
  ...
  // SJIS double-byte: split into high/low and decimal-encode as
  // `(high << 8) | low` (i.e. `high * 256 + low` per LR2 spec
  // "Shift-JIS文字コードを10進数に変換").
  const decimal = sjis;
...
/**
 * Walks every valid SJIS double-byte sequence, decodes via the
 * platform `TextDecoder('shift-jis')`, and indexes the result by
 * the resulting Unicode code point. Skips REPLACEMENT (U+FFFD)
 * results so unmapped sequences don't poison the table.
 *
 * Built lazily on first use. The cost is bounded — about 7000
 * valid SJIS pairs total, decoded in two byte loops.
 */
  ...
  for (const [highStart, highEnd] of ranges) {
    for (let high = highStart; high <= highEnd; high += 1) {
      for (let low = 0x40; low <= 0xfc; low += 1) {
        ...
        // Multi-character outputs (rare — combining marks etc.)
        // are skipped; we can't represent them as a single LR2
        // char code anyway.
        if (decoded.length > 1) continue;
        const sjis = (high << 8) | low;
        // Multiple SJIS sequences can decode to the same Unicode
        // (very rare). Keep the FIRST mapping so range-1
        // characters take precedence over range-2 duplicates,
        // matching the LR2 char-code allocation order.
        if (!map.has(code)) {
          map.set(code, sjis);
        }
      }
...
/**
 * Converts a string (UTF-16) into an array of LR2 character codes.
 * Characters that don't map to LR2 (no SJIS encoding) are returned
 * as `undefined` slots so the renderer can stamp a placeholder.
 */
