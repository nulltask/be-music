...
export { LR2_SPECIAL_GRAPHIC, isLr2SpecialGraphic } from './special-graphic.ts';
export type { Lr2SpecialGraphic } from './special-graphic.ts';
export type { Lr2PlayVariant, Lr2SkinKind } from './paths.ts';
...
function appendDestinationKeyframe(group: Lr2DestinationRect[], row: string[]): void {
  ...
  if (previous) {
    ...
    // the final, sees op4=0, and the spin code at `scene/lr2/gameplay.ts:renderImageElement` never fires.
    if (isBlank(row[21])) {
