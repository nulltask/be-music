 ...
 * plays between the song-select screen and gameplay. The skin
 * lives at `LR2files/Theme/<name>/Decide/decide.lr2skin` and
 * typically shows the song's STAGEFILE / BANNER / BACKBMP plus
 * the title / artist / genre as overlay text, animated against
 * timer 0 (scene main) keyframes.
 *
 * Lifecycle:
 *
 * - `mount(host, target)` builds the scene graph, primes the
 *   per-song texture cache, and starts the rAF loop.
 * - The scene auto-dismisses after the skin's `#STARTINPUT` ms
 *   plus a fixed window (`AUTO_ADVANCE_AFTER_STARTINPUT_MS`).
 *   Pressing Enter / Space dismisses immediately as long as
 *   `#STARTINPUT` has elapsed (LR2 spec — input is gated on
 *   that directive). Escape dismisses unconditionally.
 * - `onContinue` runs once when the scene dismisses. The
 *   demo wires it to `playSong` so dismissal kicks off the
 *   actual gameplay.
 *
 * Why a dedicated scene module: the decide skin is a different
 * `.lr2skin` from the play / select / result skins (lives at
 * `Theme/.../Decide/decide.lr2skin`), and its element op set is
 * gated on song-state flags that don't apply to the result
 * screen (e.g. difficulty / key-mode rather than clear-status).
 * Sharing `pixi-result.ts`'s renderer would have meant teaching
...
/**
 * How long to leave the decide splash on screen after
 * `#STARTINPUT` elapses before auto-advancing to play. Picked
 * to match LR2's perceived feel — short enough that hands-off
 * play kicks in promptly, long enough to read the title / artist
 * once they finish fading in.
 */
const AUTO_ADVANCE_AFTER_STARTINPUT_MS = 1200;

/**
 * Default `#STARTINPUT` for skins that omit the directive. LR2's
 * shipped decide skin uses ~1500 ms, which is also the value we
 * use for the result splash — keep them in lockstep so the
 * scene chrome / title slide-ins land at the same beat as the
 * keypress-ready cue.
 */
const DEFAULT_STARTINPUT_MS = 1500;

/**
 * Element op set flipped per-song on the decide screen. Mirrors
 * `RESULT_DYNAMIC_OPS` in `pixi-result.ts` but keeps only the
 * flags relevant to a "we're about to play this chart" splash:
 * difficulty (skins gate per-difficulty plates against these)
 * and key mode. Score-related ops (90 / 91 / 100..105) are
 * deliberately omitted — those are decided after the chart
 * runs, not at decision time.
 */
...
const DECIDE_BASE_OPS: ReadonlySet<number> = new Set<number>([
  // Per-frame "always true" flags shared with select / result.
  // 81 = "load complete" (data is ready by the time we mount the
  // decide scene), 50 = "offline" (no IR connection yet).
  50, 81,
]);
...
export interface PixiDecideViewOptions {
  skin?: Lr2Skin;
  /**
   * Fired once when the scene auto-advances OR the user
   * confirms with Enter / Space / pointer click. Hosts wire
   * this to "start gameplay".
   */
  onContinue?: () => void;
  /**
   * Fired once when the user presses Escape to bail on the
   * splash. Hosts wire this to "go back to song select".
   * Escape always works (even before `#STARTINPUT` elapses) —
   * matches LR2's bail-out behaviour. Falls back to
   * `onContinue` when not provided so a host that doesn't
   * differentiate still gets a callback.
   */
  onCancel?: () => void;
}

/**
 * Mounts the LR2 Decide skin and dismisses itself when the
 * timer / input ladder elapses. The view owns its scene-graph
 * subtree (cleared in `dispose`) and never closes the host's
 * AudioContext — decide BGM is handled by the select view, not
 * here.
 */
export class PixiDecideView {
  ...
  /**
   * Holds the per-song `#STAGEFILE` as a fullscreen sprite when
   * the chart authored one. Lives BETWEEN the solid background
   * and the skin layer so the LR2 decide chrome (title plates,
   * difficulty stickers, slide-in lines, …) paints over the
   * stagefile, matching the LR2 reference video where the
   * stagefile is the dominant visual and the skin chrome is a
   * thin overlay. Empty when the chart has no stagefile — the
   * solid background underneath then shows through.
   */
  ...
  public async mount(host: PixiSceneHost, target: PixiDecideTarget): Promise<void> {
    ...
    // Schedule timer 1 (#STARTINPUT) and the auto-advance so the
    // user sees the splash for a consistent window before
    // gameplay. Pressing Enter / Space before the start-input
    // window elapses does nothing, matching LR2's input-gating.
  ...
  private render(): void {
    ...
    // STAGEFILE backdrop. The chart-authored stagefile is the
    // standout visual on the LR2 decide screen — render it as a
    // fullscreen sprite behind whatever decide-skin chrome
    // overlays it, so themes (with or without a `*STAGEFILE`
    // skin reference) consistently get the artwork treatment.
    // No-op when the chart didn't author `#STAGEFILE` /
    // bmson `info.back_image`; the solid background underneath
    // shows through unchanged in that case.
  ...
  /**
   * Draws the chart's `#STAGEFILE` (or bmson `info.back_image`)
   * as a fullscreen backdrop. Loads the texture lazily on first
   * frame; subsequent frames pick the cached texture up from
   * `chartGraphicTextures`.
   */
  ...
  /**
   * Fallback panel rendered when the theme bundle has no
  ...
  /**
   * Resolves an LR2 special-graphic sentinel (`*BANNER` /
   * `*STAGEFILE` / `*BACKBMP`) to the texture for the focused
   * song, kicking off an async load on the first miss. The next
  ...
  private fireCancel(): void {
    if (this.continuedFired) return;
    this.continuedFired = true;
    // Fall back to `onContinue` when the host didn't wire a
    // dedicated cancel handler — better than swallowing the
    // input and stranding the user on the splash.
  ...
  private readonly handleKeyDown = (event: KeyboardEvent): void => {
    if (this.disposed) return;
    if (event.key === 'Escape') {
      // Escape always dismisses (LR2 spec — even before
      // `#STARTINPUT` elapses) and routes through `onCancel`
      // so the host can route the user back to song-select
      // instead of accidentally starting gameplay.
...
function computeDecideOps(target: PixiDecideTarget, skin: Lr2Skin): ReadonlySet<number> {
  ...
  // Difficulty plates — the LR2 default decide skin gates per-
  // difficulty stickers off these. Difficulty 0 (undefined) maps
  // to op 150 ("???") so the skin draws a generic plate when the
  // chart didn't author a `#DIFFICULTY` field.
  ...
  // Key mode — same flag IDs the select scene uses (160..164).
  // Walk the chart's events to detect 14 / 10 / 9 / 7 / 5 keys
  // since `metadata.modeHint` is bmson-only.
  ops.add(detectKeyModeOp(target.song));
  // Suppress the `Lr2Skin` argument linter — keeps the parser
  // signature future-proof if we later need to peek at the
...
function detectKeyModeOp(song: BrowserSongEntry): number {
  // Inspect playable channels in the chart to pick the right
  // key-mode flag. Mirrors the heuristic the select scene uses
