...
- `@be-music/player` は共有 engine package です。terminal 固有の挙動は `@be-music/player-tui` と [Terminal player 実装メモ](player-tui.ja.md)、browser 固有の挙動は `@be-music/player-web` と [Browser player 実装メモ](player-web.ja.md) に置きます。
