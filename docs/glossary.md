...
- **Song Select**: 曲選択画面です。譜面一覧、metadata、preview、banner、操作ヘルプを表示します。
...
- **focus key**: song select で最後に選んでいた項目を directory ごとに保存するための識別子です。通常 chart だけでなく `random` entry も含みます。
