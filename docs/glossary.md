...
- **default skin family**: Built-in browser-player skin family used when no external theme is active or when the host explicitly selects it. It supplies skinless select, decide, gameplay, and result chrome (cut-in plates in navy / ice / cyan / gold, per-element slam, diagonal wipes between scenes). Gameplay injects its chrome into the shared gameplay runtime without making LR2 import the default renderer.
