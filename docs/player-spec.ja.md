...
channel 08 経由で負の `#BPMxx` を参照すると、LR2 の片道逆スクロール・ギミックが起動します（issue #134。LR2 の根拠と意図的な逸脱は [`bms-spec.ja.md`](./bms-spec.ja.md) 参照）。

- タイミングは単調のまま: resolver は |BPM| で積分するため、全ノートは前進する `seconds` を保持し、反転前の音声スケジューリングに影響はありません。
- 反転以降、エンジンは UI フレームに鏡映した display クロック（`displaySeconds = 2 × 反転点 − 現在時刻` と対応する `displayBeat`）を載せます。TUI グリッドと web シーンはこれでスクロールするため、譜面は |BPM| 速度で逆走して見えます（チャート先頭まで巻き戻った後は先頭で固定）。ステータス時計と summary は実クロックのままです。
- 判定は反転点で凍結します: 以後は押下判定・空POOR・ミス掃き・地雷起爆は発生しません。反転を跨ぐホールドは無音で解決します（tail 判定も POOR もなし）。反転以降のノートは **未判定のまま** 終了し（POOR 掃きされません）、ゲージは保持されます。
- リアルタイム音声も同時に凍結します: 反転以降の BGM・キー音トリガは発火しません（押下による最新レーンキー音の再生は LR2 同様に残ります）。BGA は反転時点の画像を保持します。
- ステージは譜面の事前計算された末尾時刻で終了します（意図的逸脱 — 実機 LR2 は ESC まで進行不能）。playlog には `chart.reversalTimeUs` を記録し、再シミュレーションにも同じカットオフを適用します。
