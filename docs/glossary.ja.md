...
- **default skin family**: 外部 theme がない場合、または host が明示的に選ぶ場合に使う browser player 組み込みの skin family です。skinless の select、decide、gameplay、result chrome を提供します（ice-clock: navy / ice cyan / gold、要素ごとの motion、scene 間の iris cover）。gameplay では LR2 が default renderer を import しない形で共有 gameplay runtime へ chrome を注入します。
